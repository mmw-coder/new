...
def parse_args():
    parser = argparse.ArgumentParser(description="Train Router (Stage 1 & 2)")
    parser.add_argument("--stage", type=int, default=1, choices=[1, 2], help="1: Cold Start (Dexp), 2: Efficiency (Deff)")
    parser.add_argument("--data_file", type=str,default="experiments/dexp.jsonl", help="Path to training data (jsonl)")
    parser.add_argument("--save_path", type=str, default="checkpoints/router_model.pt")
    ...
    # Stage-specific defaults will be set in main if not provided
    parser.add_argument("--lambda_sparse", type=float, default=None)
    parser.add_argument("--w_ncs", type=float, default=None)
    parser.add_argument("--load_from", type=str, default=None, help="Path to pretrained model checkpoint")
...
def main():
    ...
    if args.stage == 1:
        # Stage 1: Cold Start
        # Learn to do it right. NCS weight low.
        if args.lambda_sparse is None: args.lambda_sparse = 0.01
        if args.w_ncs is None: args.w_ncs = 0.1 # Start small
        print(f"Stage 1 Training: Dexp -> Basic Routing. w_ncs={args.w_ncs}, lambda={args.lambda_sparse}")
        
    elif args.stage == 2:
        # Stage 2: Efficiency Fine-tune
        # Learn to be efficient. Higher NCS weight and sparsity.
        if args.lambda_sparse is None: args.lambda_sparse = 0.1 # Higher penalty
        if args.w_ncs is None: args.w_ncs = 1.0 # Full supervision on masks
        args.lr = args.lr * 0.1 # Lower learning rate
        print(f"Stage 2 Training: Deff -> Efficiency. w_ncs={args.w_ncs}, lambda={args.lambda_sparse}, lr={args.lr}")
    ...
    # Optimizer
    optimizer = torch.optim.Adam(graph.routing_transformer.parameters(), lr=args.lr) # Basic init
    # Add other params
    graph.add_to_optimizer(optimizer)
    # Re-create optimizer with correct params group logic from graph
    # The add_to_optimizer method adds groups to existing optimizer.
    # But we need to initialize optimizer with something first or empty.
    # Graph.add_to_optimizer adds param groups.
    # Let's clean up:
    ...
    for epoch in range(args.epochs):
        ...
        for i, sample in enumerate(pbar):
            ...
            # Target masks: use 'target_context_mask' (Deff) or 'ncs_gates' (Dexp)
            # Dpruned has 'target_context_mask'
            # Dexp has 'ncs_gates'
            ...
            # Validate lengths
            # target_agent_ids length T
            # history_trace length T-1 (or less if truncated)
            # target_masks length T-1
