...
def parse_args():
    parser = argparse.ArgumentParser(description="Train Router Stage 2: Efficiency Fine-tune (Deff)")
    parser.add_argument("--data_file", type=str, default="experiments/data/deff.jsonl", help="Path to training data (jsonl)")
    parser.add_argument("--save_path", type=str, default="checkpoints/router_deff.pt")
    parser.add_argument("--llm_name", type=str, default="Qwen/Qwen2.5-7B-Instruct")
    
    # Hyperparameters
    parser.add_argument("--epochs", type=int, default=3)
    parser.add_argument("--lr", type=float, default=1e-5, help="Learning rate (Lower for Stage 2)")
    ...
    # Stage 2 Defaults
    parser.add_argument("--lambda_sparse", type=float, default=0.1, help="Sparsity penalty (High for Stage 2)")
    parser.add_argument("--w_ncs", type=float, default=1.0, help="NCS weight (Full supervision for Stage 2)")
    
    # Mandatory Load From
    parser.add_argument("--load_from", type=str, default="checkpoints/router_dexp.pt", help="Path to Stage 1 (Dexp) checkpoint")
...
def main():
    ...
    # Load pretrained weights (Mandatory for Stage 2)
    ...
    for epoch in range(args.epochs):
        ...
        for i, sample in enumerate(pbar):
            ...
            # Target masks: use 'target_context_mask' for Deff
